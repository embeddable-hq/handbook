...
Client Context allows you to pass information about the user or environment to your React components. This is accomplished by passing a `client-context` attribute in your web component that contains an arbitrary JSON string. This can be useful for localization, theming, or other context-specific behavior. You can [learn more here](/component-libraries/client-context), including detailed examples.
 
## Complete Example

Here is a complete example of a React wrapper component (`Dashboard`) that uses the `Embeddable` component and demonstrates all of the above features. Note that the Dashboard component passes in callback functions to handle variable changes, component load events, and errors, while the Embeddable component wires up the event listeners to the web component.

First is the main `Dashboard` component that uses the `Embeddable` component:
...
Below is the `Embeddable` component, which wraps `<em-beddable>` and wires up event listeners - its helper functions then use the callback props passed from the `Dashboard` component:
