...
Note that in the above example, we're passing a callback to the Embeddable React wrapper component (see complete example below) but `variablesChange` itself does not require a callback. It's an event emitted by Embeddable whenever the variables change, and you can listen for it directly, like this:
