...
- **Z-Index Issues**: If you are embedding multiple dashboards on a single page, you may run into z-index conflicts, as each web component uses its own shadow DOM. To resolve this, ensure that each `<em-beddable>` element has appropriate `z-index` settings and is set to `position: relative`. The Embeddables that appear higher on the page must have a higher `z-index` value than those that appear lower on the page, like in this example:
