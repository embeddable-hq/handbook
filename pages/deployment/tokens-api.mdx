...
fetch('https://api.<region>.embeddable.com/api/v1/security-token', {
  ...
  body: JSON.stringify({
    ...
    user: 'sina@embeddable.com', // unique key (e.g. email or user_id) representing current user
    ...
    // external Cube
    dataProvider: 'my-cube-deployment' // any unique name you assign to your external Cube deployment
  })
})
.then((resp) => resp.json())
.then(({ token }) => {
...
- **`environment`** optionally specifies which [environment](/data/environments) to retrieve data from. Until you're ready to define environments, omit this parameter for now (by default, “default” is used). [Environments](/data/environments) let you achieve things like:
...
- **`dataProvider`** specifies which external Cube deployment Embeddable should connect to. This must match a data provider you have created using the [**Data Provider API**](/data/cube-cloud#data-provider-api) (for example, `my-cube-deployment`). 
<Callout emoji="💡">
  Note that the `dataProvider` parameter is only required when connecting to an external Cube deployment.
</Callout>
