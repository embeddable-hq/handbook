...
fetch('https://api.<region>.embeddable.com/api/v1/security-token', {
  ...
  body: JSON.stringify({
    ...
    // optional fields
    //
    customCanvasState: 'user-1-dashboard-7', // optional key used by "custom canvas" functionality to store end-user dashboard state
...
- **`customCanvasState`** is only needed if you are using [Custom Canvas](/dashboards/custom-canvas). Embeddable stores a custom canvas layout (per `embeddableId`) for each unique value of `customCanvasState` that you pass to the Tokens API.  Learn how to use this field correctly [here](./custom-canvas).  
