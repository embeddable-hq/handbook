...
- **`savedVersion`** this is the saved version of the Embeddable dashboard that you want to load.  Available options include: 
  - 'latest' (default) - the most recently saved version.  This will be used if `savedVersion` is omitted.
  - 'production' - the saved version published with the 'Production' tag
