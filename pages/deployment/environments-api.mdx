...
### **Define Connections**  
   Use the [Connections API](/getting-started/connect-your-database/connections-api#connections-api) to create named database connections (e.g. `"prod-postgres"`, `"staging-postgres"`). Each connection includes the **credentials** (host, port, user, etc.) for a particular database.
