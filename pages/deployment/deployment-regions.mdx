...
In your code repo, you'll need to update your `embeddable.config.js` file ([here](https://github.com/embeddable-hq/remarkable-pro-boilerplate/blob/main/embeddable.config.ts)) to point to the correct region:
...
When [embedding](/deployment/embedding), you'll also need to add the correct preconnect and `<script>` tag to your page's `<head>`:
