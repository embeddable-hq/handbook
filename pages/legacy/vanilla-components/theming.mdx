...
Theming Vanilla Components is a straightforward approach to styling your dashboard that allows you to control a large number of values in a single TypeScript object.
...
<Callout emoji="⚠️">
  Were you working with the previous version of Vanilla components that required you to clone that entire repo? [Go here for full instructions on switching!](theme-welcome.mdx)
</Callout>
...
Once you’ve changed `embeddable.theme.ts`, save it, and you should be set. Your theme changes will show up across all of the Vanilla Components. [Try it out](/component-libraries/build-components/defining-components#push-to-embeddable) using `npm run embeddable:dev`!

### Multiple Themes With Client Context

If you’re looking to support multiple themes, there are two separate approaches you can take: create multiple theme files, import them, and switch between them based on [client context](/component-libraries/client-context), or pass the theme values directly in client context.
...
font: {
  ...
  colorNormal: '#333942',
  colorSoft: '#e3e3e3',
  family: '"Charlie Display", Inter, Helvetica, Arial, sans-serif',
  size: '14px',
  weight: 400,
  urls: [],
},
...
**Important Note**: if your font names contain a space, you must wrap them in double-quotes (eg `"Example Font"`) inside the single quotes of the family string, as shown above, otherwise the font will not be applied correctly. Additionally, the name must match exactly what Google fonts is looking for &ndash; note that the titles on their website are not always the same as the actual font family name.
...
<Callout emoji="🚧">
  Currently, your font choice impacts the entire dashboard. In the future, with [Remarkable Pro](/component-libraries/remarkable-pro/introduction), we may support per-component and other overrides.
</Callout>
