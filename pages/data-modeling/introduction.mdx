...
<Callout emoji="💡">
  Using **Cube Cloud** or your own **Cube Core** deployment instead of Embeddable's built-in Cube? See [Bring your own Cube](/data/bring-your-own-cube) — this **Data modeling** section does not apply to you, since you define and manage your models directly in Cube.
</Callout>
...
- Cube’s SQL generation **doesn't** contain complicated witchcraft 🧙. It will always try to generate the simplest SQL it can, given your **models**, the **database type** (snowflake, postgres, etc.), the **security context** and the particular data **request**.
    - Cube is also very good at [joins](/data-modeling/writing-data-models/joins) (including multiple connected tables) and it will only perform them **if and when needed** to answer a data request.
...
- define some models ([learn more](/data-modeling/writing-data-models/defining-models)) — by hand or [with AI](/data-modeling/build-with-ai) 🚀.
- test out your models in our [data playground](/data-modeling/testing-models).
- if you're using code models, [push the models](/getting-started/development/pushing-code) to your workspace
- you can then use them to build your own dashboards in the [no-code editor](/dashboards/introduction) or as [code](/dashboards/dashboards-as-code).
- you can also learn more about:
    - joins ([here](/data-modeling/writing-data-models/joins))
    - row-level security ([here](/data-modeling/row-level-security/introduction))
