...
**Access Policies** let you control which rows of data a user can see in Embeddable by defining them in cube models or [views](https://docs.embeddable.com/data-modeling/views). They are applied at query time and affect Dashboards, Data Playground, and the SQL API.

There are two ways to do row-level security in Embeddable:

1. **SQL-based row-level security** – easy to get started, full power of SQL. Learn more [here](https://docs.embeddable.com/data-modeling/row-level-security).
2. **Access Policies** – reusable in views, applied at run time, support nested filters and role-based logic, and protect against SQL injection.

## Why use Access Policies

SQL-based [row-level security](https://docs.embeddable.com/data-modeling/row-level-security) works, but it introduces **several problems at scale**.
...
- **Safety**: Automatically escape values from the `securityContext`, protecting against SQL injection.
...
- The `customers_view` brings in all columns from the `customers` model. The access policy is applied at the view level, so you only need to define it once instead of repeating it across every model.
...
In some cases, you may want to restrict access entirely by default and then selectively allow it for specific roles. You can do this by setting `includes: []` for the `*` role:
...
- Prefer defining access policies in v**iews** and keep cubes private. This reduces duplication and makes policies easier to maintain.
- Keep role names stable; map them from your app’s auth layer.
