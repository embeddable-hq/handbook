...
There are three ways to do row-level security in Embeddable:

1. **Security filters** – the simplest way to ensure row-level security is applied globally across all your data models. Pass filters in the [Tokens API](/deployment/tokens-api) and Embeddable enforces them on every query automatically. Learn more [here](/data-modeling/row-level-security#security-filters).
...
3. **Access Policies** – a more scalable approach to row-level security if you start using [Views](/data-modeling/views) and want role-based security.
...
- **[Security filters](/data-modeling/row-level-security#security-filters)** – the simplest way to enforce row-level security globally across all your data models, without any changes to your SQL.
