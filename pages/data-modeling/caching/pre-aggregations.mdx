...
Pre-aggregations allow you to compute and store aggregations (like sums, averages, and counts) in advance rather than calculating them on the fly for each query. When a query matches the structure of a pre-aggregation, results are retrieved directly from the pre-aggregated table instead of scanning raw data, significantly speeding up response times (and reducing cost and load on your databases).
