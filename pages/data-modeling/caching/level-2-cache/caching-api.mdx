...
<Callout emoji="💡">
  The `savedVersion` value for a given dashboard tells Embeddable which models and cube version it should use when refreshing pre-aggregations (e.g. it will use those tagged as “production”)
   ...
   - `apiKey`: your [API key](/getting-started/development/pushing-code#pushing-using-your-api-key)
   ...
   - `securityContext`: a JSON object [passed to your data models](/data-modeling/row-level-security/sql-based-security) to enforce row-level security. When setting up scheduled refreshes for pre-aggregations, provide a list of securityContext objects to the scheduledRefreshContexts array in the API. This ensures Cube can generate the required security contexts before running the refresh. Learn more [here](https://cube.dev/docs/product/configuration/reference/config#scheduled_refresh_contexts).
   - `filters`: optional. An array of filter objects (each with `member`, `operator`, and `values`) matching those passed to the [Tokens API](/deployment/tokens-api). Ensures the cache is built with the correct filters applied for that security context.
...
<Callout emoji="💡">
`Non-scheduled` security contexts are built once. Cube won’t refresh them or check their refresh keys.
</Callout>
