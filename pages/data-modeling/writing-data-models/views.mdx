...
- Define a **clean facade** in front of your data models.
...
- **Clutter**: Views allow you to represent different **subsets** of your connected Cubes as individual Views (e.g. you could have just a Marketing view, a Sales View and an Admin view).  You can even have different names, for each measure and dimension, for each View.
- **Ambiguous join paths:** Views represent a subset of the connected Cubes and thus allow you to specify exactly which join path to use for any measure or dimension that is queried using that View.
...
**Access policies** allow you to apply both `member_level` and `row_level` security to your Views based on roles (i.e. control which columns and rows are visible to which end-users).  This is a nice alternative to **cube-level [row-level security](/data-modeling/row-level-security/introduction)**, as:
...
Learn more [here](/data-modeling/row-level-security/access-policies).