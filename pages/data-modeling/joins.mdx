...
You can use the `sql` parameter to combine tables, like so:
...
- This approach is often very useful if multiple raw database tables need to be joined together to represent one ***clean*** **user-facing concept**. 

- Following that, the general rule of thumb is to try and have **one data model** for **each user-facing concept**. 
...
### Multiple Models with Joins

Since `customers`, `orders`, and `products` are **user-facing concepts**, the common approach would be to give each table its own model and then link them:
