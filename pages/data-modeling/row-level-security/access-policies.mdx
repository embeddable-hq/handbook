...
**Access Policies** let you control which rows of data a user can see in Embeddable by defining them in [cube models](/data-modeling/writing-data-models/defining-models) or [views](/data-modeling/writing-data-models/views). They are applied at query time and affect [dashboards](/dashboards/introduction) and [data playground](/data-modeling/testing-models).

## Why use Access Policies

[SQL-based security](/data-modeling/row-level-security/sql-based-security) works well at the model-level, but it introduces **several problems at scale**:
...
Here's a simple example of an access policy defined on a [view](/data-modeling/writing-data-models/views).

**Token security context:**

You can specify `roles` in your `security-context.sc.yml` ([learn more](/data-modeling/row-level-security/access-policies#testing-your-access-policies)) like so:
...
- The `customers_view` brings in all columns from the `customers` model. The access policy is applied at the view level, so you only need to define it once instead of repeating it across every model. Learn more about views [here](/data-modeling/writing-data-models/views).
...
Each filter uses the same `member`, `operator`, and `values` fields as [Security Filters](/data-modeling/row-level-security/security-filters#filter-fields) — refer there for the full operator reference.
...
- You can define data access policies in both cubes and views. We recommend [keeping cubes private](/data-modeling/writing-data-models/views#how-to-define-cube-views) and defining access policies only in views. This makes policies easier to maintain and reason about, especially for member-level access.
...
## Testing your Access Policies

Define presets in your repo at `src/presets/security-contexts.sc.yml` ([here](https://github.com/embeddable-hq/remarkable-pro-boilerplate/blob/main/src/embeddable.com/presets/security-contexts.sc.yml)) and switch between them in the **"View as"** dropdown to test how roles affect visibility.
...
- Prefer defining access policies in [views](/data-modeling/writing-data-models/views) and [keep cubes private](/data-modeling/writing-data-models/views#how-to-define-cube-views). This reduces duplication and makes policies easier to maintain.
...
- **[SQL-based row-level security](/data-modeling/row-level-security/sql-based-security)** – the recommended approach if you want control of row-level (or schema / table-level) security at the model level.
