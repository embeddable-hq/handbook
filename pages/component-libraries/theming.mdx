...
Themes are objects that allow Embeddable users to control the global configuration of their components. Embeddable's component library, [Remarkable Pro](/component-libraries/remarkable-pro/introduction), leverages Embeddable's theming system to provide a consistent look and feel across all components. Remarkable Pro also allows the theme to control date ranges, internationalization, and some component functionality.

## themeProvider

`embeddable.theme.ts` defines and exports a `themeProvider` function. The role of `themeProvider` is to decide which values to apply to the theme based on the incoming [Client Context](/component-libraries/client-context), and return the merged theme using `defineTheme(parentTheme, newTheme)`, which combines customizations with the existing theme.
...
```tsx
// embeddable.theme.ts
import { Theme, defineTheme } from '@embeddable.com/remarkable-pro';
