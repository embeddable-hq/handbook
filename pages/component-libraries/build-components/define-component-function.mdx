...
- **`inputs`**: Input fields users configure (e.g. `strings`, `numbers`, `datasets`, as well as [custom types](/component-libraries/build-components/custom-types)).
- **`defaultWidth` / `defaultHeight`**: Initial size in the builder (in pixels, snapped to a grid).
- **`events`**: Specifies **events** the component can trigger (e.g. `onChange`, `onClick`), which is key to interactivity. Learn more about events [here](/component-libraries/build-components/interactivity). 
- **`variables`**: Auto-creates **variables** in the no-code Builder when the component is used, which are key to interactivity. Learn more about variables [here](/component-libraries/build-components/interactivity).
...
- **`measure`** represents a [**measure**](/data-modeling/writing-data-models/dimensions-and-measures) (count, sum, etc.), as defined in your data models.
- **`dimension`** represents a [**dimension**](/data-modeling/writing-data-models/dimensions-and-measures) (e.g. group by country or category), as defined in your data models.
- **`granularity`** for grouping time dimensions in [`loadData`](/component-libraries/build-components/loading-data#types).   
    ```ts
...
Events are used to create [interactive components](/component-libraries/build-components/interactivity). For example, a dropdown containing a list of countries that, when clicked, filters the data. Or a set of buttons showing 'day', 'week', and 'month' that, when clicked, update the x-axis granularity on a time-series chart. 
...
Variables are a core-concept in Embeddable. They are used mainly to store the values passed to Embeddable from events fired by [interactive components](/component-libraries/build-components/interactivity). 
...
- **`props(inputs, [state, setState], clientContext)`**  
  ...
  - **`state`** and **`setState`** are state local to the `.emb` file, enabling you to pass state from your react component back to the `.emb.` file. The most common use-case is to [load data dynamically](/component-libraries/build-components/loading-data#loading-data-dynamically) from within the component.
  - **`clientContext`**: Any [additional data](/component-libraries/client-context) passed in from the web component at embed time (e.g. theming, environment info).  
    ...
    ```
- **`events`** describes how events your component triggers are transformed into a data object that Embeddable can use for [interactivity](/component-libraries/build-components/interactivity), as described in `events.properties` in `meta`.
