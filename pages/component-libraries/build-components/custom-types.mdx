...
Out of the box, we provide you with all the standard types, like `string`, `number`, `boolean`, `time` and `timeRange` (we call these [native types](/component-libraries/build-components/define-component-function#native-input-types)). But sometimes you'll want to create your own **custom types**.
...
The `defineType` function is how you define a **custom type** in Embeddable (on top of the built-in [native types](/component-libraries/build-components/define-component-function#native-input-types)).
...
- **`optionLabel`**: This tells Embeddable how to render each **option** of this custom type in the UI ([learn more](/component-libraries/build-components/custom-types#the-defineoption-function)).
- **`cast`**: an object specifying how to convert this custom type to other Embeddable types, so variables of this type can be used in component inputs of different types ([learn more](/component-libraries/build-components/custom-types#casting-between-types)).
- **`toNativeDataType`**: an object specifying how to map this custom type to Cube-native data types, for use in **dataset filters** only ([learn more](/component-libraries/build-components/custom-types#using-custom-types-to-filter-datasets)). For component input interoperability, use `cast` instead.
...
The **second argument** is the javascript **value** that represents that option (e.g. `{ name: 'Orange', r: 256, g: 158, b: 84 }`).  This is the javascript value that will get passed to a component if that option is chosen (concrete example of this [below](/component-libraries/build-components/custom-types#using-a-type-in-a-component)).
<Callout emoji="💡">
...
## Extending native types with additional properties

You can also use `defineOption` to add options to native types, which can be useful if you want to extend a native type with additional properties. The process is the same as for custom types, but instead of passing a custom type as the first argument, you pass a native type (e.g. `string`, `number`, etc.). This can include more complex native types like `time` and `timeRange` too.

As an example, if you wanted to add `Last Three Weeks` as an option to the native `timeRange` type, you could do it like so:

```ts
// timeRangeOptions.type.emb.ts
...
You don't need to import this type into your components the way you would with a true custom type, as it's just an additional option on the existing `timeRange` type.  Once defined, it will be available in any dropdowns of the `timeRange` type across your Embeddable builder (e.g. Variable builder, component inputs, etc.)
...
The fastest way to use custom types is to use `defineOption` (as demonstrated [above](/component-libraries/build-components/custom-types#the-defineoption-function)) as it automatically makes this type available as a **dropdown** of the available options:
...
<Callout emoji="💡">
  `cast` works for **component inputs**. If you also need your custom type to work in **dataset filters**, you'll need `toNativeDataType` too — see [below](/component-libraries/build-components/custom-types#using-custom-types-to-filter-datasets).
</Callout>
