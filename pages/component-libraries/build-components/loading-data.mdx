...
Below is an `.emb` file for a **bar chart** component that calls `loadData` inside the [`defineComponent`](/component-libraries/build-components/defining-components) function:
...
Below are the main parameters relevant to `loadData(...)`, with examples. For each type, you can prompt the user to select it by defining the appropriate [inputs](/component-libraries/build-components/defining-components#inputmeta-definition) in your `.emb.ts` file.
...
- `from` is a `Dataset` which is defined in the no-code builder.  It defines which [data model](/data-modeling/writing-data-models/defining-models) to use, as well as any filters that should be applied to the query.


- `select` is an array of [dimensions](/data-modeling/writing-data-models/dimensions-and-measures#dimensions), [measures](/data-modeling/writing-data-models/dimensions-and-measures#dimensions) and *time dimensions* (time-based dimensions, often paired with a **granularity**).
...
- `offset` is a number specifying how many rows to skip, e.g. for [pagination](/component-libraries/build-components/loading-data#1-pagination-example). Defaults to 0.
...
<Callout emoji="💡">Note that the JavaScript `Date` object operates in the local time zone of the user's environment or in UTC and doesn't natively support time zones. When developing your own components, it's easy for dates to accidentally get converted out of the intended time zone, so it's important to check thoroughly to be sure they're passing the correct timestamp and time zone combination to `loadData`.</Callout>
...
- **`onTheDate`**: Property falls on a specified date.  
- **`beforeOrOnDate`**: Property is before or on a specified date/time.  
- **`afterOrOnDate`**: Property is after or on a specified date/time.
...
Sometimes you need to fetch new data triggered by an action or event fired from within your React component. You can achieve using [**Embeddable State**](/component-libraries/build-components/defining-components#definecomponent-config-reference). 
