...
├── definition.ts               # Overrides meta, config, preview (required)
├── MyCustomBarChart.emb.ts     # SDK integration (required)
└── index.tsx                   # React component (only if you need to customise the UI)
```

The `definition` file contains the following definition object, enabling to override any part:
...
| `previewConfig` | Editor canvas preview | Show new inputs in preview |


This means you can extend the specific parts you want, re-using the rest of the component and logic. 
