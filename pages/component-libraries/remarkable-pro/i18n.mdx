...
Namespace: `theme.i18n`

In Remarkable, you can localize language via theming:
...
You can see the full list of labels in the Remarkable [English translation file](https://github.com/embeddable-hq/remarkable-ui/blob/main/src/remarkable-pro/theme/i18n/translations/en.ts). You can translate these into other languages by providing translations using the format shown [above](/component-libraries/remarkable-pro/i18n#language).  
...
Some labels come from the theme rather than the core components.

For example, each export option has a label (e.g. “download as CSV”).
...
To do this, add translations under `translation.dimension`. 

You can use either:

- **Qualified key**: "data-model-name.field.value" (recommended for safety)
- **Direct match**: "Value"

**Example**

Imagine you have a data model called **customers** containing **a dimension** called **country,** which returns values like *United States* and *Germany*:
...
- At render time, Remarkable UI looks up the value and replaces it.
