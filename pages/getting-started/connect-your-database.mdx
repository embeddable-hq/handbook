...
You can connect one or multipe databases to Embeddable.

- **Read-only access:** Embeddable only reads from your database - it never writes to it.
- **Connection options:** Connect directly or via SSH for secure access.
- **IP whitelisting:** Restrict access to your database by allowing only Embeddable’s fixed IPs.


## Connections API

Use our **Connections API** to link your database to your workspace. 

The below represents a `CREATE` action, but all `CRUD` operations are available, with sample scripts available in [your repo](https://github.com/embeddable-hq/vanilla-components/tree/main/src/scripts).
...
- **name:** Unique name for the database connection. Default models look for a connection named `default`.
- **type:** Supported drivers include Postgres, BigQuery, MongoDB, etc. (See full list [here](https://cube.dev/docs/product/configuration/data-sources)).
- **credentials:** A javascript object containing the necessary credentials expected by the driver.
...
Restrict database access by allowing only Embeddable’s IPs:
