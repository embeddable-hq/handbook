...
Embeddable ships with [Remarkable Pro](/component-libraries/remarkable-pro/introduction), a full suite of components, but you can build anything you need on top.
...
1. Inside `src/embeddable.com/components` in your [repo](/getting-started/get-set-up/set-up-your-workspace), create a new folder called `ExampleDropdown`
```bash
mkdir src/embeddable.com/components/ExampleDropdown
```
2. And create file called `index.tsx`.  
```bash
touch src/embeddable.com/components/ExampleDropdown/index.tsx
```
...
1. In the `ExampleDropdown` folder, create a second file called `ExampleDropdown.emb.ts`. 
```bash
touch src/embeddable.com/components/ExampleDropdown/ExampleDropdown.emb.ts
```
...
This spins up a **local workspace** that uses your local components and models ([learn more](/getting-started/development/local-environment)). 

### Add your component to a dashboard

1. Create a new dashboard, or open the one you made in the [previous step](/getting-started/get-set-up/first-embeddable). 
2. Click the **Add component** button. You should now see your "Example dropdown" there! 
...
Learn more about [defining components](/component-libraries/build-components/defining-components), [loading data](/component-libraries/build-components/loading-data) and [adding interactivity](/component-libraries/build-components/interactivity).

## Next Steps

🎉 Congratulations! You have completed the getting started guide, and learned about the core concepts of Embeddable!

Take a break and treat yourself! Once you're back, why not look at one of the following topics:

- [Connect your database](/data/connect-your-database)
- [Create your first data model](/data-modeling/introduction#data-models) to define and manage data access 
- [Apply row level security](/data-modeling/row-level-security/introduction) so your end-users see the right data
- [Set up your theme](/component-libraries/remarkable-pro/theming) to match your brand 
