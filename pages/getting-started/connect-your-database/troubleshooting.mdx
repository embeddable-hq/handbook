...
2. **Firewall or Network Restrictions** 
    - Ensure your database server allows incoming connections from Embeddable’s [IP addresses](/getting-started/connect-your-database#ip-whitelisting).  
    - If you’re hosting on-prem or behind a VPN, confirm that Embeddable can reach the server.

3. **SSL or TLS Requirements**  
   - Some databases require SSL. Confirm that you’ve enabled `ssl: true` (or relevant SSL parameters) [in your credentials](/getting-started/connect-your-database/credentials) if needed.  
...
6. **Data Source Configuration**  
   - In your model file (`<name>.cube.yml`), verify the `datasource` field references the correct connection in your settings (if nothing is specified, it will point at Embeddable's default demo database).
