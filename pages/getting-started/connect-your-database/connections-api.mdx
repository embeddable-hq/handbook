...
- `credentials`: A javascript object containing the credentials expected by the driver.
    * The necessary credentials for each database are available [here](/getting-started/connect-your-database/credentials). 
...
To confirm your connection works, use the `/test` endpoint, which is also available as a [script](https://github.com/embeddable-hq/vanilla-components/tree/main/src/scripts) in your repo. 
...
- To use different databases for environments (e.g., production, QA, test) or customers, configure connections with the [Environments API](/deployment/environments-api).
