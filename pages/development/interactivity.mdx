...
You can also use **dimensions** and **measures** as variables. This is useful for components that need to filter data based on user selections, like a dropdown that lets users pick a dimension or measure to display. In order to facilitate this behavior, we provide six predefined picker components that can be accessed in the no-code builder:

- **SelectorDimension**: A single-select dropdown for dimensions.
- **SelectorMeasure**: A single-select dropdown for measures.
- **SelectorDimensionOrMeasure**: A single-select dropdown that lets users choose between dimensions and measures.
- **MultiSelectorDimension**: A multi-select dropdown for dimensions.
- **MultiSelectorMeasure**: A multi-select dropdown for measures.
- **MultiSelectorDimensionOrMeasure**: A multi-select dropdown that lets users choose between dimensions and measures.

These components automatically create variables in the no-code builder, allowing users to select dimensions or measures that can be used to filter datasets or drive other components.
...
<ImageGrid images={["/img/dim-meas-vars-inputs.png"]}/>
