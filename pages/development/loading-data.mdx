...
- `timezone` is the timezone you want to aggregate the **time dimensions** by.  Must be a string in [tz database format](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones), e.g. 'America/Los_Angeles', 'Europe/Paris', 'Australia/Sydney', 'UTC', etc. Defaults to 'UTC'.  If you're using [Caching API](/data-modeling/caching/caching-api), note that [pre-aggregations](/data-modeling/caching/pre-aggregations) are timezone aware, and so, for example, a query run with timezone 'Europe/Paris' cannot use a pre-aggregation built with timezone 'America/New_York'.
