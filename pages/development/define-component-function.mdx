...
These values are then passed to your component along with the other input props, and can be used within the component to customize behavior. Here's an example of a table with no custom inputs set:

<ImageGrid images={["/img/subinputs-table-before.png"]} />

And here's the same table using the custom inputs defined in the screenshot of the no-code builder above:
