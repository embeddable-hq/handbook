...
<Callout emoji="⚠️">
To use theming, you must upgrade to Vanilla Components v1. Full information on how to do so can be found [on our upgrading page](/development/theme-upgrading).
</Callout>

### The Basics

If you use Vanilla Components, you will be provided with a complete default theme under the hood (the same colors and appearance you can see on our example components page), and can then override as many or as few values as you want, within what the theme supports. This is a much larger variety of values than was previously available to change, and we’ll list them all in an appendix to this document, but if all you want to do is change a few chart colors and the font, your theme object could look like this:
...
Let’s break down what’s happening here. First, we’re bringing in the `Theme` type from Vanilla Components to help with type consistency, and the `defineTheme` function from the same repo, which we use to help us merge our changes into the default Vanilla Components theme. Our `themeProvider` function has `clientContext` (which we’re not using in this example, but see below!) and `parentTheme` parameters. Without that `parentTheme`, we wouldn’t get all of the values from Vanilla Components. You could still go without it, but you’d have to define every single property specified in `Theme` which, as of this writing, is about 130 lines of code. Should you want that level of granular control, just skip using `defineTheme` at all, and return your theme object directly.

Once you’ve changed `embeddable.theme.ts`, save it, and you should be set. Your theme changes will show up across all of the Vanilla Components.
...
Under the hood, our theme provider attempts to find any custom fonts you define in `theme.font.family` via Google Fonts. If they support it, we can load it. If you’re trying to use another font provider, please contact our Customer Success team for further information.
