...
export default defineComponent(Component, meta, {
  ...
  events: { // maps events fired by your component onto a JS object defined in events.properties in meta
    onChange: (v) => ({ value: v }), 
  },
});
```
...
| `meta`      | [Meta](/development/defining-components#definecomponent-meta-reference)      | ✅ Yes       |
| `config`    | [Config](/development/defining-components#definecomponent-config-reference)   | ✅ Yes       |
...
### `defineComponent` Component Reference
...
### `defineComponent` Meta Reference
...
### `defineComponent` Config Reference

**`config`** is the third argument to `defineComponent`. It controls how inputs become props and how local `.emb` state (Embeddable State) is managed.

| **Param** | **Type**                                              | **Required** |
| --------- | ----------------------------------------------------- | ------------ |
| `props`   | `(inputs: Inputs<typeof meta>, [state, setState]) => any` | ✅ Yes       |
| `events`  | `{ [eventName: string]: (payload: any) => any }`      | ❌ No        |

- **`props`** receives `(inputs, [state, setState])`, where:
  - **`inputs`** are the values entered by the user in the no-code builder.
...
- **`events`** describes how events your component triggers are transformed into a data object that Embeddable can use, as described in `events.properties` in `meta`.
