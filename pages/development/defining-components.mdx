...
Some inputs may directly reference the **dimensions** and **measures** you've defined in your data models, which may not have the most readable names. You can define additional inputs that will be used to generate extra inputs that you can reference in your code. They look like this in the builder UI:

<ImageGrid images={["/img/subinputs-builder-example.png"]} />

These additional inputs will be passed to your component as props, just like the top-level inputs. You can then reference them in your component code, allowing you to create more complex and customizable components (or just make things easier to read, like this table).
...
The way Embeddable allows you to do this is by defining an additional `inputs` array within any given item the top-level `inputs` array. Here's a quick example of an input with its own sub inputs:
...
inputs: [
  // The top-level input
  {
    ...
    array: true,
    inputs: [
      // The sub-inputs for the 'Columns' input
      {
        name: 'label',
        type: 'string',
        label: 'Label name',
...
When you do this, the Embeddable SDK recognizes it and will automatically create expandable inputs in the builder UI, like this:
 
The available sub-input types are:
