...
<Callout emoji="⚠️">
  Custom types are still in **Beta**.  You may experience **typescript errors** when using them, you may need to restart `embeddable:dev` for changes to take effect, and custom types are not yet supported in **Custom Canvas**. We may make **breaking changes** to them in the future.
</Callout>
...
Let's define a `Color` type.

Create a file named `Color.type.emb.ts` (file must end in `.type.emb.ts` for Embeddable to pick it up):
...
**Things to notice:**
- `defineType` is what tells Embeddable that we're introducing a new type (on top of the standard [native types](/development/define-component-function#native-input-types)).
- The first argument to `defineType` (in this case `'color'`) is a unique identifier for the type. This name is used internally by Embeddable to reference the type in dashboards where it is used. Updating it once it's in use can potentially break your dashboards.
- `label` is the human-readable name that appears in the UI (you can change it anytime without impacting the underlying logic):
<ImageGrid width='50%' images={["/img/custom-type-label.png"]} />

## The `defineOption` function

The easiest way to introduce a new type is to provide it with a few **options**.  Think of this like an `enum`.  You can use **options** to statically define the values available for this type.
...
**Things to notice**:
- The first argument to `defineOption` is the **type** itself.  This tells Embeddable which **type** you're adding the **option** to.
- The second argument to `defineOption` is the javascript value that represents that option.  That means that if a component takes an **input** of type **Color** (we'll show how to do this [below](/development/custom-types#using-a-type-in-a-component)) and the builder chooses "Orange", then `{ name: 'Orange', r: 256, g: 158, b: 84 }` is the value that will be passed to the component (concrete example of this [below](/development/custom-types#using-a-type-in-a-component)).
<Callout emoji="💡">
  In this case we're using `name` and `r`, `g` and `b` to represent our type, but we could define the shape of the object however we like.  As long as the components that use this type know how to handle it.
</Callout>
- Also notice the `optionLabel` on the `defineType` function.  This tells Embeddable how to render the **option** in the UI (in this case Embeddable will use the `name` property from `{ name: 'Orange', r: 256, g: 158, b: 84 }`):
<ImageGrid width='50%' images={["/img/custom-type-values.png"]} />

## Using a type in a component
...
Let's define a **very** basic RGB (Red-Green-Blue) editor for our `Color` type:

<ImageGrid width='50%' images={["/img/color-editor.png"]} />

It will allow builders to define their colors using custom RGB values, both in the Variable builder (shown above) and inside components themselves:
...
**Things to notice**:
- The `defineEditor` function tells Embeddable that this editor exists (must be called from inside a file ending in `.emb.ts`).
- The first argument to `defineEditor` is the React component we defined above (the one that will be used as the editor for this custom type).
- The `type` in the `meta` is the custom type that this editor should be used to edit.
...
- The `value` tells us the current value.
