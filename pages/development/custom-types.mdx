...
Out of the box, we provide you with all the standard types, like `string`, `number`, `boolean`, `time` and `timeRange` (we call these [native types](/development/define-component-function#native-input-types)). But sometimes you'll want to create your own **custom types** or extend the existing native types.
