...
<Callout emoji="💡">
  Building [as code](/dashboards/dashboards-as-code)? Variables work exactly the same way — define them in the [`variables`](/dashboards/dashboards-as-code#variables) block of your `.embeddable.yml` and bind them to inputs and datasets just as you would in the builder.
</Callout>
...
### A variable gets updated  
   ...
   - Some components [create variables automatically](/component-libraries/build-components/interactivity#usage-flow-1) when you add them to the Canvas, but you can also create variables manually. 
...
That’s the core model: an action updates a variable, and everything connected to it reacts.

<Callout emoji="🎥">
  Want to see variables in action, check out our [Quick Hits video tutorial on Variables](/video-tutorials/quick-hits/variables) for a step-by-step walkthrough.
</Callout>
...
In order to facilitate this behavior, we provide six predefined picker components in [our starter library](/component-libraries/remarkable-pro/introduction) that can be accessed in the no-code builder. These components automatically create variables, allowing users to select dimensions or measures that can be used to filter datasets or drive other components.
...
Alongside `text`, `number`, `time`, and the rest, Embeddable also supports a native **`filters`** variable type. Where a normal variable holds a single value, a `filters` variable holds an entire filter *expression* — one or more conditions combined with `AND`/`OR`, including nested groups. This is what powers filter-builder style components, letting an end user construct arbitrary filter logic (e.g. “Country equals *United States* AND (Status equals *Active* OR Status equals *Pending*)”) and apply the whole thing to a dataset in one go.
...
```ts
type QueryFilter = { property: Dimension | Measure; operator: string; value: unknown };
...
### Creating a `filters` variable

1. In the **Variables** panel, click the **+ icon** to create a new variable.
2. Give it a name and set its **type** to **Filters**.

<ImageGrid width='50%' images={["/img/filters-as-variables-add.png"]}/>

Unlike other variable types, a `filters` variable has no default value to configure — it always starts out empty (`{}`) until something sets it.

### Setting the variable from a component

Components that build filter expressions — like a filter-builder UI — emit their result through an event, which you connect to your `filters` variable the same way you would [connect any other variable to a component event](#example-click-a-pie-chart-segment-to-filter-data).

<Callout emoji="💡">
  Building your own filter-builder component? Declare a `filters`-typed event property and variable in your `.emb.ts` file:
  ...
  See [Building Interactive Components](/component-libraries/build-components/interactivity) for more on wiring up events and variables.
</Callout>

### Applying a `filters` variable to a dataset

Once a `filters` variable holds a value, you can apply it to a dataset in either of the usual places:
